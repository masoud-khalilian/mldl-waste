...
class ICNet_f_f(SegBaseModel):
    """Image Cascade Network"""

    def __init__(self, nclass=cfg.DATA.NUM_CLASSES, backbone='resnet50', pretrained_base=False):
        super(ICNet_f_f, self).__init__(nclass, backbone, pretrained_base=pretrained_base)
    ...
    def forward(self, x):
        ...
        x_sub4 = F.interpolate(x, scale_factor=0.5, mode='bilinear', align_corners=True)
        _, _, _, x_sub4 = self.base_forward(x_sub4)
        x_sub4 = self.ppm(x_sub4)
        outputs = self.head(x_sub1, x_sub1, x_sub4)
...
class _ICHead(nn.Module):
    def __init__(self, nclass, norm_layer=nn.BatchNorm2d, **kwargs):
        ...
        self.cff_12 = CascadeFeatureFusion(64, 32, 32, nclass, norm_layer, **kwargs)
        self.cff_24 = CascadeFeatureFusion(2048, 32, 64, nclass, norm_layer, **kwargs)
