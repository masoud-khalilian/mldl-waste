...
def resnet50_v1s(pretrained=False, root='~/.torch/models', **kwargs):
    model = ResNetV1b(BottleneckV1b, [1, 2, 3, 1], deep_stem=True, **kwargs)
    if pretrained:
...
class ICNet_f_h(SegBaseModel):
    """Image Cascade Network"""

    def __init__(self, nclass=cfg.DATA.NUM_CLASSES, backbone='resnet50', pretrained_base=False):
        super(ICNet_f_h, self).__init__(nclass, backbone, pretrained_base=pretrained_base)
        self.conv_sub1 = nn.Sequential(
            _ConvBNReLU(3, 32, 3, 2),
            _ConvBNReLU(32, 32, 3, 2),
            # _ConvBNReLU(32, 64, 3, 2) removed to decrease the size of the encoder
        )
...
class _ICHead(nn.Module):
    def __init__(self, nclass, norm_layer=nn.BatchNorm2d, **kwargs):
        ...
        self.cff_12 = CascadeFeatureFusion(1024, 32, 256, nclass, norm_layer, **kwargs)
        self.cff_24 = CascadeFeatureFusion(2048, 512, 1024, nclass, norm_layer, **kwargs)
        self.conv_cls = nn.Conv2d(256, nclass, 1, bias=False)
