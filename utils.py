from torch import int as torch_int
...
def _fast_hist(label_true, label_pred, n_class):
    ...
    hist = np.bincount(
        n_class * label_true[mask].to(torch_int) +
...
def scores(label_trues, label_preds, n_class):
    ...
    acc_cls = np.divide(np.diag(hist), hist.sum(axis=1), out=np.zeros_like(
        np.diag(hist)), where=hist.sum(axis=1) != 0)
    acc_cls = np.nanmean(acc_cls)

    intersection = np.diag(hist)
    union = hist.sum(axis=1) + hist.sum(axis=0) - intersection
    iu = np.divide(intersection, union, out=np.zeros_like(
        intersection), where=union != 0)
